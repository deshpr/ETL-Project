...
#time output=`curl -s -H "Content-Type: application/json" -X POST -d  $json https://g8ehey0fmc.execute-api.us-east-1.amazonaws.com/helloSQLite` 
...
time output=`aws lambda invoke --invocation-type RequestResponse --function-name helloSqlite --region us-east-1 --payload $json /dev/stdout | head -n 1 | head -c -2 ; echo` 
echo ""
echo "AWS CLI RESULT:"
echo $output | jq
echo ""
